...
- `cgroup/sendmsg4` + `cgroup/sendmsg6` — close the UDP gap that `connect4`/`connect6` alone leave open: a datagram socket that never calls `connect()` and instead calls `sendto()`/`sendmsg()` directly reaches the kernel via `BPF_CGROUP_UDP4_SENDMSG`/`BPF_CGROUP_UDP6_SENDMSG`, a separate attach point from `BPF_CGROUP_INET4_CONNECT`/`BPF_CGROUP_INET6_CONNECT`. Both hooks receive the same `struct bpf_sock_addr *` context as their connect counterparts, so they share `check_ipv4`/`check_ipv6` verbatim — no separate policy path to drift.
...
- **Reporter** — writes per-connection verdict lines to stdout in a stable format: `verdict=<V> pid=<P> tgid=<T> comm=<C> dst=<domain> (<ip>):<port>`. Verdict values: `ALLOW`, `DENY(no-domain)`, `DENY(not-in-policy)`, `SKIP(dns)`, `SKIP(loopback)`, `SKIP(self)`. `SKIP(self)` takes priority over every other verdict (`cmd/agent/main.go`'s `verdictForEvent`) when the event's TGID matches the agent's own PID: the agent makes no outbound connections of its own beyond the DNS lookups `seedAllowlist` performs at startup, and Go's resolver issues one connect() per candidate resolved address as an internal RFC-6724-style address-preference probe — never sending real DNS or application data — which the connect() tracepoint captures like any other event and attributes to a real-looking destination (e.g. a CDN IP for an allowlisted domain) the agent never actually talked to. Diagnostics go to stderr via `log/slog`; keep the stdout format stable — CI smoke tests grep it, and `internal/report.ParseLine` parses it back (round-trip tested against `Line.String()`).
...
12. UDP enforcement gap closed: `cgroup/sendmsg4`/`sendmsg6` attached alongside `connect4`/`connect6` so unconnected UDP (`sendto()`/`sendmsg()` without `connect()`) is subject to the same allowlist instead of silently bypassing block mode ✅ (monitoring/logging of unconnected UDP is still connect-tracepoint-based and does not yet cover this path — see the known-limitations note)
