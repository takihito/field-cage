...
- Exports `FC_BIN` and `FC_LOG` (and `FC_MODE` when explicitly overridden) via `GITHUB_ENV` so a later `report` sub-action step in the same job can reuse the verified binary and log path without repeating inputs; `log-file`/`mode` are rejected if they contain a newline before being written, since a raw multi-line value would let it inject an extra `GITHUB_ENV` record that later steps (including `if: always()` ones) would then run with
- **`report/action.yml`** — a separate composite sub-action (composite actions can't run a `post:` step, so this is invoked explicitly with `if: always()` at the end of the job) that shells out to `field-cage report --format markdown` for the job summary and `--format annotations` for per-denial annotations, and exposes `denied-count`/`allowed-count`/`suggested-allowlist`/`log-file` as step outputs via `jq` over `--format json` (same newline-rejection as above applies to the log path before it is written to `GITHUB_OUTPUT`, since this sub-action can also run standalone with a caller-supplied `log-file`). The `upload-log` artifact step reuses the same resolved `log-file` output rather than re-deriving the default path, so a custom log path is archived correctly. The raw log is not dumped into the job's own log by default — pass `dump-log: true` to opt in — per the "prefer summarized logs over raw dumps" guidance in `AGENTS.md`.
