...
- **Unconnected UDP is blocked but not yet logged**: block mode enforces the allowlist against a UDP socket regardless of whether it calls `connect()` or sends directly via `sendto()`/`sendmsg()` (see "Block mode enforcement model" above). Monitoring is currently connect-tracepoint-based only, so a denied or allowed `sendto()`/`sendmsg()` call that never connects does not appear in the verdict log, even in audit mode — enforcement and visibility are not yet symmetric for this path.
