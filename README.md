...
Block mode is **default-deny**: the `cgroup/connect4` and `cgroup/connect6` programs reject every outbound IPv4/IPv6 connection unless its destination IP is on the allowlist. UDP is covered by the same policy on both paths a datagram socket can take: `cgroup/connect4`/`connect6` handle a socket that calls `connect()`, and `cgroup/sendmsg4`/`sendmsg6` handle one that calls `sendto()`/`sendmsg()` directly without ever connecting — a separate kernel attach point that `connect4`/`connect6` alone cannot see, so both must be enforced or unconnected UDP would silently bypass the allowlist. IPv4-mapped IPv6 destinations (`::ffff:a.b.c.d`, the path dual-stack runtimes such as Node.js and Java take to IPv4 hosts) are checked against the IPv4 allowlist, so one IPv4 entry covers both socket families. The allowlist is built by:
...
- **Unconnected UDP is invisible to monitoring in both modes, and enforced only in block mode**: monitoring is connect-tracepoint-based only, so a `sendto()`/`sendmsg()` call that never calls `connect()` never appears in the verdict log — in audit mode this means that traffic is invisible outright, not merely unenforced. In block mode the cgroup/sendmsg4 and cgroup/sendmsg6 programs (see "Block mode enforcement model" above) still enforce the allowlist against it despite the log staying silent, so enforcement and visibility are asymmetric there specifically: a denied or allowed unconnected UDP datagram is invisible in the log yet still blocked or permitted.
...
│    (block mode) → default-deny; allows      │
│      loopback, port 53 to trusted resolvers,│
