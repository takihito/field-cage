...
- IPv4 CIDR サブネット指定（例: `10.0.0.0/8`, `203.0.113.0/24`）
...
>
> **CIDR**: IPv4 CIDR のみ対応しています。CIDR エントリは eBPF の LPM trie に直接シードされるため、サブネット内の全アドレスが DNS 解決なしで許可されます。
...
- **ライブ観測は平文 IPv4 UDP DNS（port 53）のみ**: IPv6 トランスポート・TCP・暗号化（DoH/DoT）の DNS は観測できないため allowlist を拡張できません。それらで解決されるドメインは起動時シードのみが対象となり、起動後に IP がローテーションすると新しい IP を block モードが拒否します（fail-closed）。該当ドメインはポリシーで IP 固定するか、平文 IPv4 UDP で解決されるようにしてください。
