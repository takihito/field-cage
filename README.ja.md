...
- YAML ポリシーによるドメインの完全一致指定または `*.example.com` 形式のワイルドカード指定、IP アドレス（IPv4/IPv6）指定（いずれも大文字小文字不問）
...
allowlist:
  ...
  - "*.githubusercontent.com" # ワイルドカード: サブドメインにのみマッチし、ベースドメイン自体には非マッチ
...
> **注意**: ワイルドカードは `*.example.com` の形式のみ対応し、必ずセカンドレベルドメインまで指定してください。`*.com` / `*.jp` のような裸の TLD ワイルドカードや、それ以外の形（`api.*.com`、`*` など）はポリシー読み込み時にエラーになります。ワイルドカードはサブドメインにのみマッチします（`*.example.com` は `example.com` 自体には一致しません。必要であれば別途列挙してください）。ワイルドカードエントリは起動時に事前解決できません。通常のドメインエントリと異なり起動時シードによる救済も無く、該当ドメインの DNS 応答を実際に観測できた場合*のみ* allowlist に反映されます。
>
> **マルチテナントゾーンへのワイルドカード指定に関する注意**: `*.s3.amazonaws.com`、`*.github.io`、`*.pages.dev` のように第三者が自由にサブドメインを作成できるドメインに対してワイルドカードを指定すると、そのゾーン配下に名前を作れる任意の第三者が自分の IP を実質的に allowlist へ追加できてしまい、allowlist の目的そのものを損ないます。ワイルドカードは自社または信頼できるベンダーが完全に管理しているゾーンにのみ使用してください。
>
...
- **connect しない UDP は遮断されるが、まだログには出ない**: Block モードでは UDP ソケットが `connect()` を呼ぶか、`connect()` せず直接 `sendto()`/`sendmsg()` するかにかかわらず同じ allowlist が適用されます（上記「Block モードの enforcement モデル」参照）。一方で監視は現状 connect tracepoint ベースのみのため、一度も `connect()` しない `sendto()`/`sendmsg()` の許可・拒否は Audit モードであっても verdict ログに出力されません。この経路については enforcement と可視性がまだ非対称です。
